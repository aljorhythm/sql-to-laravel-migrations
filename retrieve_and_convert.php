...
if (count($only_include_tables) > 0) {
	$table_names = array_filter($table_names, function ($table_name) use ($table_names, $only_include_tables) {
		return in_array($table_name, $only_include_tables);
	});
} else {
	$table_names = array_filter($table_names, function ($table_name) use ($table_names, $exclude_tables) {
		return !in_array($table_name, $exclude_tables);
	});
}
...
class $classname extends Migration
{
	...
	public function up()
	{
		Schema::connection('$connection_name')->create('$table_name', function (Blueprint \$table) {
		$table_schema_codes
		});
	}
